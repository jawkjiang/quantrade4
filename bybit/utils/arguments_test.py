...
class ArgumentsTest:
    ...
    def refine_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 筛选价格0.01到100的货币对
        available_keys = [key for key in data.columns if key != 'timestamp']
        selected_symbols = [symbol for symbol in available_keys if 0.01 <= float(tickers[symbol]['lastPrice']) <= 100]
        # 计算data中所有货币对过去五天价格的波动率
        waving_rates = {}
        for symbol in selected_symbols:
            if symbol == 'timestamp':
                continue
            waving_rate = data[symbol].pct_change().std()
            waving_rates[symbol] = waving_rate
        # 选取波动率最大的60个货币对
        selected_symbols = sorted(waving_rates, key=waving_rates.get, reverse=True)[:60]
        # 过滤数据框架以仅包含选择的60个货币对加上timestamp
        data = data[['timestamp'] + [symbol for symbol in selected_symbols if symbol in data.columns]]
        return data

        # Deprecated at 10/25/2024
