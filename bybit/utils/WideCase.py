...
class WideCase:
    def __init__(self, http: HTTP):
        ...
        self.group = {}
        self.difference_history = []    # 每次交易差额记录，时序填入，时序弹出，每个单元为一个dict，包含symbol, difference, timestamp
        ...
        # 创建正在flush的flag，避免update方法在flush进行时执行
    ...
    def flush(self):
        ...
        with open('../data/wide_group.json', 'r') as f:
            group = json.load(f)
        # self.group保留前30个币对，或盈利率在20%以上的币对，两者取交集
        group1 = {key: group[key] for key in list(group.keys())[0:30]}
        ...
        # Trim self.difference_history，只保留最近48h的数据
        current_time = datetime.datetime.now()
        self.difference_history = [each for each in self.difference_history if current_time.timestamp() * 1000 - each['timestamp'] < 172800000]
        # 找到difference最小的币对，将其加入freezing_symbols
        if len(self.difference_history) > 0:
            min_difference = min([each['difference'] for each in self.difference_history])
            if min_difference < 0:
                symbol = [each['symbol'] for each in self.difference_history if each['difference'] == min_difference][0]
                self.freezing_symbols[symbol] = current_time.timestamp
    ...
    def update(self, timestamp: int):
        ...
        with self.lock:
            try:
                ...
                if len(self.data) == 50 and not self.FLUSH_FLAG:
                    # 使用self.data撮合交易
                    self.trade()
                ...
                for symbol in list(self.freezing_symbols.keys()):
                    if current_time.timestamp() * 1000 - self.freezing_symbols[symbol] > 345600000:  # 96小时
    ...
    def entry(self, seat: Seat) -> None:
        ...
        # 有时候会遇到ab not enough的问题，估计是全仓杠杆超过10倍了，这里试一下自动减仓，如果现有的position开不了就减半，两次以上减半就不开了。
        for i in range(3):
            try:
                http.place_order(category='linear', symbol=symbol, side='Buy' if direction == 'long' else 'Sell',
                                 qty=position, orderType='Market')
                break
    ...
    def indicator_calculate(self, seat: Seat) -> None:
        ...
        difference = 0
        if seat.side == 'long':
            difference = seat.entry_margin * (current_price - seat.entry_price) / seat.entry_price
        elif seat.side == 'short':
            difference = seat.entry_margin * (seat.entry_price - current_price) / seat.entry_price
        self.difference_history.append({
            'symbol': seat.symbol,
