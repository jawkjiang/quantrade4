"""
    初筛的实测版本。代码和回测版本几乎一致，只是将回测的数据传入改为了传入的data。
"""
...
A_Win_Rate = [0.63, 7 / 8, 6 / 7, 1]
A_Trade_Times = [9, 8, 7, 4]
B_Win_Rate = [0.37, 1 / 8, 1 / 7, 0]
B_Trade_Times = [9, 8, 7, 4]
A_Win_Rate_Threshold = 15
B_Win_Rate_Threshold = 15


# 注意面对复杂的函数功能时，应先写好函数的框架，再把框架中的功能分解到其它子函数中实现
def primary_filter(data: pd.DataFrame, tick: int) -> tuple[list[str], list[str]]:
    """
    初筛。
